...
import CarouselItemGLeague from "./CarouselItemGLeague";
import CarouselNavItemGLeague from "./CarouselNavItemGLeague";

export default class PlayerCarouselGLeague extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      nav1: null,
  ...
  getDefense(players) {
    for (var i = 0; i < players.length; i++) {
      ...
      var scaledDef = this.scaleStat(
        -96.0,
        parseFloat(player.dbpm) * -1,
        -115.0
      );
      // var dbpm = parseFloat(player.dbpm);
      // var dws = parseFloat(player.dws);
      var defRating = scaledDef;
  ...
  renderNav() {
    ...
    //     <div>
    //       <CarouselItemGLeague player={player} />
    //     </div>;
    //   });
    // }
  }
